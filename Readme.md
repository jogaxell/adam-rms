...
- **Storage Location when adding assets** — the *Add Asset* page has a Storage Location picker (sub-locations indented), which stays selected for *Add Another*. A batch of new kit is filed where it lives straight away, without editing each asset afterwards.
