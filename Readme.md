...
- **Server config form actually saves** — the JWT key field says "leave blank to keep existing value" but was still marked required, so the browser blocked every submission and no server setting could be changed once a key was set. Blank now means keep, as the field always claimed.
